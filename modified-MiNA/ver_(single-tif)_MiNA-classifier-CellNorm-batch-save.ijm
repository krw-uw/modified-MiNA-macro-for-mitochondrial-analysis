...
//Instructions
//-NOTE RUNS WELL ON MAX PROJ OF 3 CONFOCAL SlICES 200NM APART, 100x, deconvolution optional, mito561, nuclei-405>>This V12 has line 113 modified to run-z-projection on substack
...
//    currently set at default 0.5 (see line 490 to change). Alternativly retrain model to avoid ambiguous regions. Comment out line 500 to avoid manual intervention. 
...
var UNSHARP_STRENGTH = 0.6;
  ...
  function processFile(path) {
       if (endsWith(path, ".tif"))  {  //can change to .nd2
       		run("Bio-Formats", "open=["+path+"] autoscale color_mode=Default display_metadata rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT swap_dimensions z_1=1 c_1=2 t_1=1"); //added split channel for tif
			imageName = getTitle();
...
function crudeSegment(){
		...
		run("Options...", "iterations=2 count=1 black pad do=Erode");			//Erode to undo previous dilation
