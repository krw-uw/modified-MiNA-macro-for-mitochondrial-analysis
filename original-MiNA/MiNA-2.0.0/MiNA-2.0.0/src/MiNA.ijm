...
// kurt modified for 3d, still need to auotmate the few steps asking for input - choose 'black background' only with 'defaults' selected, then process stack. 
// removed all the 'close' feature to view progress. Use additional 'waitForUser commands
//Global Variables--------------------------------------------------------------
//Output Arrays...
...
macro "MiNA - Analyze Morphology" {
	...
	run("Duplicate...", "title=TestSkeleton duplicate");
	...
	selectWindow("Original");
	run("8-bit");
	//maybe do combine colors or add images, or doesn't matter? 
	// only worsk for 2d //run("Add Image...", "image=TestSkeleton x=0 y=0 opacity=100 zero");
	run("Merge Channels...", "c2=TestSkeleton c4=Original create keep");
    ...
    run("Merge Channels...", "c4=Original c6=Outline create keep");
	...
	//kurt added wait for user for ability to move the slider
...
function preprocessing() {
	...
	selectWindow("Original");
	if (UNSHARP == true) {
		run("Unsharp Mask...", "radius="+toString(UNSHARP_RADIUS)+" mask="+toString(UNSHARP_STRENGTH) +" stack");
	}

    //Apply contrast limited adaptive histogram equalization if selected
	if (CLAHE == true) {
		run("Enhance Local Contrast (CLAHE)",
                    "blocksize="+toString(CLAHE_BLOCKSIZE)+" histogram="+toString(CLAHE_HISTOGRAM)+" maximum="+toString(CLAHE_MAXSLOPE)+" mask=*None* fast_(less_accurate) stack");
	}

    //Apply median filtering if selected
	if (MED == true) {
		run("Median...", "radius="+toString(MED_RADIUS)+" stack");
	}
